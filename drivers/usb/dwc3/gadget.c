...
static int dwc3_gadget_set_ep_config(struct dwc3 *dwc, struct dwc3_ep *dep,
		...
		bool ignore, bool restore, u32 cfg_action)
{
	struct dwc3_gadget_ep_cmd_params params;
		...
		bool ignore, bool restore)
{
	...
	ret = dwc3_gadget_set_ep_config(dwc, dep, desc, comp_desc, ignore,
			restore, DWC3_DEPCFG_ACTION_INIT);
	if (ret)
		return ret;
		...
		int is_selfpowered)
{
	...
	if (dwc->soft_connected == is_selfpowered)
		return 0;

	dwc->soft_connected = is_selfpowered;
		...
		for (epnum = 2; epnum < DWC3_ENDPOINTS_NUM; epnum++) {
			...
			if (dep->endpoint.desc)
				dwc3_gadget_set_ep_config(dwc,
					...
					false, false, DWC3_DEPCFG_ACTION_RESTORE);
